...
const longText = `Dawn threads a pale gold through the alley of glass.
...
async function main(): Promise<void> {
  ...
  for (const part of parts) {
    const summary = await mapChain.invoke({ context: part.pageContent });
  ...
  const result = await reduceChain.invoke({ context: combinedSummaries });
  console.log(result);
}
